...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	/* Extra Variables I used */
	int candidate_lenght_in_codons = candidate_length / 3;
	int j,k, j_shifter =0;

	/* This is the variable that will control the outer for loop, or in otherwords, this variable 
	will control how many times we shift the candidate */
	int shifter = candidate_lenght_in_codons - sample_length_in_codons;  
	

	// Controls the shifting of candidate, j controls how many shifts we will do , j_shifter is how much we shift by
	for (j = 0; j <= shifter; j ++)			 
	{
		// Controls which condon in sample we will compare in the if statements below
		for (iterations = 0; iterations < sample_length_in_codons; iterations++)		
		{
			// Compare each codon in the sample with the respective codon in the candidate
			// The 3 * iteration controls which codon in the sample/candidate we are currently checking
			if (strncmp((sample_segment + 3 * iterations), (candidate_segment + 3 * iterations+j_shifter), 3) == 0)	
				temp_score += 10;
	
			// Call the is_same_acid funtion to see if the two codons have the same name
			else if (is_same_acid( (sample_segment + 3 * iterations), (candidate_segment + 3 * iterations+j_shifter) ))
				temp_score += 5;
			
			else
				// This loop steps through the individual codons, used for nucleotide comparisons
				for (k = 0; k < CODON_LENGTH; k++)
				{
					// Checks for nucleotide matches
					if (*(sample_segment + 3 * iterations + k) == *(candidate_segment + 3 * iterations +j_shifter+ k))
						temp_score += 2;

					// Check if specified nucleotides are a base pair by calling is_base_pair
					else if (is_base_pair(*(sample_segment + 3 * iterations +k), *(candidate_segment + 3 * iterations + j_shifter+k)))
						temp_score += 1;
				}
		}

		// If this is the largest score so far store it, then shift the candidate and check a new score
		if (temp_score > score)
			score = temp_score;
		
		// Reset temp_score to 0, and shift the candidate's position by a codon
		temp_score = 0; 
		j_shifter += 3;
	}
...
/*
 * Compares sample and candidate strings and determines if the first codon in both share the same amino acid
 * 
 * PARAM:  pointer to a segment
 * PARAM:  pointer to a candidate
 * PRE:    pointers are valid pointers dynamically allocates in memory. 
 * POST:   passes ownership of pointer memory back to caller
 * RETURN: 1 if the two share the same amino acid else returns 0
 */
int is_same_acid(char* sample_segment, char* candidate_segment) 
{
	// Index of the codon, which will correspond to an index in the codon_names array
	int i_sample, i_candidate;

	i_sample = get_codon_index(sample_segment);
	i_candidate = get_codon_index(candidate_segment);

	// Check to see if the strings at the corresponding index match
	if (strcmp(codon_names[i_sample], codon_names[i_candidate])==0)
		return 1;
	else
		return 0;
}