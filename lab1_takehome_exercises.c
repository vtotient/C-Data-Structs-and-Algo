...
Author:			  Atoosa Karimi, Victor Sira
Student #s:		11667169 and 21278163
CS Accounts:	l5p1b and u1k1b
...
int main(void)
{
  ...
  FILE * file_pointer = NULL;               /* A "pointer" to a file */
   ...
   fft(DIR, EXPONENT, omega, complex_component);
                                            ...
                                            */
   for (i = 0; i < NUMBER_OF_READINGS; i++) {
	   readings[i] = pow((pow(readings[i],2)+pow(complex_component[i],2)),.5);
   }
   ...
   for (i = 0; i < NUMBER_OF_READINGS/2; i++) {
	...
	fprintf( file_pointer, "Puddlejump Tidal Freqency: %f", frequency);
