...
int main(void)
{
	...
	FILE * file_pointer = NULL;               /* A "pointer" to a file */
											  ...
											  /* Start by initializing the elements in the tidal frequency array so that each omega[i] equals
	...
	for (i = 0; i < NUMBER_OF_READINGS; i++) {
		omega[i] = (float)i*SAMPLING_FREQUENCY / NUMBER_OF_READINGS;
	}
	...
	*/
	for (i = 0; i < NUMBER_OF_READINGS / 2; i++) {
		if (omega[i] >= NOISE_FILTER && omega[i] > frequency)
			frequency == omega[i];
